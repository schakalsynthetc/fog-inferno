#!/! /w/eb/c/httpd/m4.b /limbo/httpd/t
#
# httpd builtin m4
#
implement M4;
	...
	include "draw.m";
i	nclude "bufio.m"; bufio: Bufio; Iobuf: import bufio;
	include "sh.m";
	include "arg.m";
	include "cache.m";
	include "contents.m";
...
File: adt { name:string; line:int; fp: ref Iobuf; };
...
init(g: ref Private_info, req: Httpd->Request)
{
	...
	err:string; err=send (g,cgiparse->cgiparse(g,req));
	if(err=nil){ 
		g.bout.flush (); 
	} else {
		oops(g,cgidata,cgiparse->cgiparse(g,req), err);
	}
}

oops (g: ref Private_info, cgidata: ref CgiData, err:string): string
{
	g.bout.puts (cgidata.httphd); g.bout.puts (sys->sprint(
		"<html><body><h1 class='oops'>%s</h1></body></html>",
		err
	));
	return err;
}

send (g: ref Private_info, cgidata: ref CgiData): string
{	
	if(cgidata==nil){ g.bout.flush(); return; }
	g.bout.puts (cgidata.httphd);
	pushfile(cgidata.uri, g.bout)); 
	scan(); 
	return nil; 
}
