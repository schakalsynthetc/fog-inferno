...
multi: int; nlines: int; stderr, stdout: ref FD;
...
line (fd: ref FD, file: string, toss; int): int
{
	p, n, m: int;
	...
	for(m=0; ; ){
		c:= array[1] of byte;
		n = read(fd, c, 1);
		if(n < 0){ fprint(stderr, "getline: error reading %s: %r\n", file); exit; }
		if(n == 0){ break; }
		buf[m++] = c[0];
		if(int c[0] == '\n') {p++; break;}
	}
	if (toss==0) write (stdout,buf,m); return m;
}

offset(fd: ref FD, file: string)
{ do{if(line(fd, file,1)==0) break; }while (--offset>0); }

lines(fd: ref FD, file: string)
{ do{if(line(fd, file,0)==0) break; }while (multi || --nlines>0); }
