...
include "draw.m"; draw: Draw; Font: import draw;
include "bufio.m";
include "acmewin.m"; win: Acmewin; Win, Event: import win;
include "string.m"; str: String;
include "readdir.m"; readdir: Readdir;
include "names.m"; names: Names;
include "plumbmsg.m"; plumbmsg: Plumbmsg; Msg: import plumbmsg;
Leaf: module {
...
showleaf (win: ref Win, dlp : array of ref Dirlist, ndl : int)
{ columnate(win,dlp,ndl); }

columnate (win: ref Win, dlp : array of ref Dirlist, ndl : int)
{
	i,j,w, colw,mint,maxt,ncol,nrow: int ; dl : ref Dirlist;
...
mainwin (w: ref Win)
{
	c := chan of Event; na:int; ea:Event; s:string;
	...
	loop: for(;;){
		...
		'x' or 'X' =>
			...
			if(n == 0)
				...
				break loop;
		'l' or 'L' =>
			eq := e; if(e.flag & 2) eq =<-c;
			s = string eq.b[0:eq.nb];
			if(eq.q1>eq.q0 && eq.nb==0)
				s = w.wread(eq.q0, eq.q1);
			n := dolook(w, s);
			if(n == 0) w.wwriteevent(ref e);
		}
	}
	postnote(1, pctl(0, nil), "kill"); w.wdel(1); exit;
}
