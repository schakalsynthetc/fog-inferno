; 			Oleg Standard Scheme "Prelude"
; Version for ULisp, adapted from myenv-chez.scm by Stéphane Rollandin 
; Oleg's amazing repository is there: http://okmij.org/ftp/
...
;;======================================= main ULisp changes below

(require 'pretty-print)
(require 'format)

;; ULisp does not support syntax-case
...
;; load assert.scm if the following is not enough:
...
(define ulisp-error error)
(define error
  (lambda (msg . args)
    (ulisp-error (format "~a~%" (cons msg args)))))

; used by lambda-calc.scm :
(define-macro (++ x) `(setq ,x (+ 1 ,x)))

;;======================================= no more ULisp changes below
	  ...
	  (if res
	    ...
	    (begin
	      ...
	      default-exp)))))
			...
			; Only feature-identifiers srfi-0 and
			; ulisp are assumed predefined.
			; See below why this
			; syntax-rule may NOT use an let-syntax.
(define-syntax cond-expand
  (syntax-rules (else ulisp srfi-0 and or not)
    ((cond-expand)
      (error "Unfulfilled cond-expand"))
    ((cond-expand (else . cmd-or-defs*))
      (begin . cmd-or-defs*))
    ((cond-expand "feature-id" ulisp kt kf) kt)
