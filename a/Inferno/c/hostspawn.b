...
init (nil: ref Draw->Context, args: list of string)
{
	...
	if(str==nil) fail(sys->sprint("cannot load %s: %r", String->PATH));
	arg := load Arg Arg->PATH;
	if(arg==nil) fail(sys->sprint("cannot load %s: %r", Arg->PATH));
	...
	args=arg->argv(); if(args==nil)arg->usage(); arg=nil;
	...
	if (cfd == nil) fail(sys->sprint("cannot open /cmd/clone: %r"));
