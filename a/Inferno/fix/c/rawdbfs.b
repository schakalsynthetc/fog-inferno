...
include "sys.m"; sys: Sys; Qid: import Sys;
include "draw.m";
include "arg.m";
include "styx.m"; styx: Styx; Rmsg, Tmsg: import styx;
...
Serve: while ((gm := <-tchan)!=nil) {pick m:=gm {
	Readerror =>
	...
	Read =>
		(c, err) := srv.canread(m);
		if(c == nil) { srv.reply(ref Rmsg.Error(m.tag, err)); break; }
		if(c.qtype & Sys->QTDIR){ srv.read(m); break; }
	case TYPE(c.path) {
		Qindex =>
			if(database.index < 0) {
				srv.reply(ref Rmsg.Error(m.tag, Eperm)); break;
			}
			if (m.offset > big 0) {
				srv.reply(ref Rmsg.Read(m.tag, nil)); break;
			}
		...
		Qstats =>
			...
			reply := array of byte sys->sprint("%d %d %d %d", database.s_reads, database.s_writes, database.s_creates, database.s_removes);
	...
	Write =>
		...
		if(!database.ownlock(c)) {
			...
			srv.reply(ref Rmsg.Error(m.tag, Elocked));break;
		}
	case TYPE(c.path) {
		Qindex =>
			if(database.index >= 0) {
		...
		Qdata =>
			...
			if(recno == -1)
				...
				if(changed && database.write(recno, m.data) == -1){
					srv.reply(ref Rmsg.Error(m.tag, sys->sprint("%r")));break;
				}
				if(changed)
	...
	Clunk =>
		c := srv.getfid(m.fid); if(c!=nil) database.unlock(c); srv.clunk(m);
	Remove =>
		c := srv.getfid(m.fid); database.unlock(c);
...
open (srv: ref Styxserver, m: ref Tmsg.Open): ref Fid
{
	...
	if(c == nil){ srv.reply(ref Rmsg.Error(m.tag, err)); return nil; }
	
	if(TYPE(c.path) == Qnew){
		# generate new file
		if(c.uname != user){
			srv.reply(ref Rmsg.Error(m.tag, Eperm)); return nil;
		}
		r := database.create(array[0] of byte);
		if(r == nil) {
			srv.reply(ref Rmsg.Error(m.tag, "create -- i/o error")); return nil;
		}
	...
	if(database.locking && TYPE(c.path) == Qdata && (m.mode & (Sys->OWRITE|Sys->ORDWR))) {
		if(!database.lock(c)) {
			srv.reply(ref Rmsg.Error(m.tag, Elocked)); return nil;
		}
	}
...
dirslot(n: int): int
{
	for(i := 0; i < len database.records; i++){
		...
		if(r != nil && r.datalen >= 0){if(n == 0) return i; n--;}
	}
	return -1;
}
...
dirgen (p: big): (ref Sys->Dir, string)
{
	...
	Qnew =>
		return (dir(Qid(QPATH(0, Qnew),0,Sys->QTFILE), "q", big 0, user, 8r600), nil);
	Qindex =>
...
{
	while((m := <-navops) != nil) {pick n := m {
	Stat =>
		n.reply <-= dirgen(n.path);
	Walk =>
		if(int n.path != Qdir){
			n.reply <-= (nil, "not a directory"); break;
		}
	case n.name {
		".." =>
			;	# nop
		...
		* =>
			if(len n.name < 1 || !(n.name[0]>='0' && n.name[0]<='9')){	# weak test for now
				n.reply <-= (nil, Enotfound); continue;
			}
			n.path = QPATH(int n.name, Qdata);
		}
		n.reply <-= dirgen(n.path);
	Readdir =>
		if(int m.path != Qdir){ n.reply <-= (nil, "not a directory"); break; }
		o := 1;	# Qnew;
		stats := -1;
		indexing := -1;
		if(database.indexing)
...
{ if(db.updcmd != nil) sh->system(context, db.updcmd); }
...
{
	if (TYPE(c.path) != Qdata || !db.locking) return;
	ll := db.locklist;
	db.locklist = nil;
	for(; ll != nil; ll = tl ll){
		lock := hd ll;
		...
		} else db.locklist = hd ll :: db.locklist;
	}
}
...
freerec(db: ref Database, recno: int)
{
	...
	for (i:=recno;i>=0;i--) if (db.records[i].datalen != -1) break;
	f:=i+1;
	nb:=0;
	for (i=f;i<nr;i++){ if (db.records[i].datalen != -1) break;
...
allocrec(db: ref Database, nb: int): int
{
	...
	for(i:=0; i<len db.records; i++) {
		r:=db.records[i]; if(r.datalen==-1) {
			avail:=r.count-nb;
			if (avail>=0 && (n==-1 || avail<best)) {best=avail;n=i;}
		}
	}
	if (n!=-1) return n;
	nr:=len db.records; a := array [nr+1] of ref Record ;
	a[0:]=db.records[0:]; offset:=0;
	if (nr>0) offset = a[nr-1].offset+a[nr-1].count ;
	...
	a[nr] = Record.new(offset+HEADLEN,nb) ; db.records=a;
	return nr;
}
