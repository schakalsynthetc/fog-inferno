...
init (nil: ref Draw->Context, argv: list of string)
{
	...
	path:=hd tl argv; (dir,f):=pathsplit(path) ;

	if (sys->bind("#s",dir,Sys->MBEFORE|Sys->MCREATE)==-1) {
		sys->fprint(stderr,"ramfile: %r\n"); return;
	}
	
	fio:=sys->file2chan(dir,f); if(fio==nil){
		sys->fprint(stderr, "ramfile: file2chan failed: %r\n"); return;
	}

	data:=array[0] of byte; if (tl tl argv !=nil) data=array of byte hd tl tl argv ;
	
	spawn server (fio,data) ;data=nil; 
}

server (fio: ref Sys->FileIO, data: array of byte) { for (;;) alt { 

	(offset, count, nil, rc) := <-fio.read => if (rc!=nil) {
		if (offset>len data) 
			rc <-= (nil,nil); 
		else {
			end:=offset+count; if (end>len data) end=len data;
			rc <-= (data[offset:end], nil);
		}
	}
	
	(offset, d, nil, wc) := <-fio.write => if (wc!=nil) {
		if (offset==0) data=array[0] of byte;
		end:=offset+len d;
		if (end>len data) {
			ndata:=array [end] of byte; ndata[0:]=data; data=ndata; ndata=nil;
		}
		data[offset:]=d;
		wc <-= (len d, nil);
	}
...
{
	for (i:=len p-1;i>=0;i--) if (p[i]!='/') break; if (i<0) return (p,nil);
	p=p[0:i+1]; for (i=len p-1;i>=0;i--) if (p[i]=='/') break; if (i<0) return (".",p); 
