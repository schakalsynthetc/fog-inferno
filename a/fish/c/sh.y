...
LIBSHELLRC: con "/x/fish/x/init";
BUILTINPATH: con "/x/fish/b";
...
init(drawcontext: ref Draw->Context, argv: list of string)
{
	initialise();
	opts:=blankopts;
	if (argv!=nil) { if((hd argv)[0] == '-') opts.lflag++; argv=tl argv; }
	interactive := 0;
...
loop: while (argv != nil && hd argv != nil && (hd argv)[0]=='-') {
		for (i := 1; i < len hd argv; i++) {
			c := (hd argv)[i];
			case c {
			'i' => interactive = Context.INTERACTIVE;
			'l' => opts.lflag++;	# login (read $home/lib/profile)
			'n' => opts.nflag++;	# don't fork namespace
			'e' => opts.ctxtflags |= Context.ERROREXIT;
			'x' => opts.ctxtflags |= Context.EXECPRINT;
			'c' => arg: string;
				if (i < len hd argv - 1) {
					arg = (hd argv)[i + 1:];
				} else if (tl argv == nil || hd tl argv == "") {
					usage();
				} else {arg=hd tl argv; argv=tl argv;}
				argv=tl argv; 
				opts.carg=arg;
				continue loop;
			}
	...
	if (!opts.nflag) sys->pctl(Sys->FORKNS, nil);
	ctxt := Context.new(drawcontext);
	ctxt.setoptions(opts.ctxtflags, 1);
	if (opts.carg != nil) {
		status := ctxt.run(stringlist2list("{" + opts.carg + "}" :: argv), !interactive);
		if (!interactive) { if(status!=nil) raise "fail:" + status; exit; }
		setstatus(ctxt, status);
	}

	# if login shell, run standard init script
	if (opts.lflag) runscript(ctxt, LIBSHELLRC, nil, 0);

	if (argv == nil) {
		if (opts.lflag) runscript(ctxt, SHELLRC, nil, 0);
		if (isconsole(sys->fildes(0))) interactive |= ctxt.INTERACTIVE;
...
{ initialise(); lex := YYLEX.initstring(s); return doparse(lex, "", 0); }
...
{{
	...
	if (fd != nil) runfile(ctxt, fd, path, args);
	else if (reporterr)
			ctxt.fail("bad script path", sys->sprint("sh: cannot open %s: %r", path));
} exception e {
	"fail:*" => if(!reporterr) return; raise;
}}
...
Redirword: adt { fd: ref Sys->FD; w: string; r: Redir; };

Redirlist: adt { r: list of Redirword; };
...
{
	if (n == nil || n.ntype != n_PIPE) return n;
...
listjoin(left, right: list of ref Listnode): list of ref Listnode
{
	...
	for (; left != nil; left = tl left) l=hd left :: l ;
	for (; l != nil; l = tl l) right=hd l :: right ;
	return right;
}
...
{ if (ctxt.options() & Context.VERBOSE) sys->fprint(stderr(), "sh: %s\n", s); }
